...
def run_delly(delly_path, output_dir, reference_path, sampleID, cancerBam, normalBam, delly_maskFile, dryrun):
    ...
    outputbaseName = cancerID + '_' + normalID + '_delly.bcf'
...
def run_manta(python2_path, manta_path, output_dir, reference_path, sampleID, cancerBam, normalBam, dryrun):
    ...
    # output ID has to be unique since output files are all identical, only
    # differentiated by folder name
    outputID = f'{sampleID}_{sampleNameBam(cancerBam)}_{sampleNameBam(normalBam)}'
    manta_outputdir = os.path.join(output_dir, outputID)
...
def run_strelka(python2_path, strelka_path, output_dir, reference_path, sampleID, cancerBam, normalBam, manta_indel_candidates, dryrun):
    ...
    strelka_outputdir = os.path.join(output_dir, outputID)
...
def mpileup(samtools_path, bamfile, reference_path, dryrun):
    ...
    return os.path.abspath(mpileup_output)


def run_varscan(java_path, varscan_path, samtools_path, tabix_path, bgzip_path, output_dir, cancerBam, normalBam, reference_path, dryrun):
    ...
    varscanCMD = f'{java_path} -jar {varscan_path} somatic {normal_mpileup} {cancer_mpileup} {output_basename} --output-vcf 1 --strand-filter 1'
    execute(varscanCMD, dryrun)

    snp_vcf, indel_vcf = f'{output_basename}.snp.vcf', f'{output_basename}.indel.vcf'

    #create vcf.gz file
    bgzip_tabix(bgzip_path, tabix_path, snp_vcf, dryrun)
    bgzip_tabix(bgzip_path, tabix_path, indel_vcf, dryrun)


    # delete mpileups after using
    # execute(f'rm -rf {normal_mpileup} {cancer_mpileup}', dryrun)
...
def main(cancer_bam, normal_bam, reference_fasta, multi_core, multi_thread, config_path, output_dir, sample_info_path, sampleID, list_of_tools, dryrun):
    ...
    for sample_id, cancer_path, normal_path in sample_list:
        ...
        if 'varscan' in list_of_tools:
            varscan = run_varscan(paths['java'], paths['varscan'], paths['samtools'], paths['tabix'], paths['bgzip'], output_dir_tools[
